...
#| fig-align: 'center'
#| fig-cap: 
#|   - "Guinea pig tooth length between OJ and VC"
#|   - "Guinea pig tooth length between dosages (0.5 mg/day, 1 mg/day, 2 mg/day)"
...
# Perform the t-test
t_test_result <- t.test(len ~ supp, data = ToothGrowth, var.equal = FALSE, alternative = "two.sided")
...
Since the p-value was 0.06063 which is greater than 0.05, we fail to reject the null hypothesis which means that there is no statistically significant evidence that treatment mean tooth length for the OJ supplement delivery method is different than the treatment mean tooth length for the VC supplement delivery method. The study also suggests, since the confidence interval is from -0.1710156 to 7.5710156 which includes 0, that there is no significant difference between the two delivery methods.

**Question 13** -- Create another section header, describing the contents of this section.

## Challenge 1

Make the following changes to your document: Specify “code-folding” in your YAML document options. Add a table of contents to your document. Add a color theme to your document.
